...
/**
 * Remove index matching the given `path`.
 *
 * @param {Route} path
 * @return {Boolean}
 * @api public
...
/**
 * Lookup index defined with a path
 * equivalent to `path`.
 *
 * @param {Stirng} path
 * @return {Array}
...
/**
 * Lookup index matching the given `url`.
 *
 * @param {Stirng} url
 * @return {Array}
 * @api public
 ...
 *
 * Param mapping is used to provide pre-conditions to index
...
methods.forEach(function(method){
  app[method] = function(path){
    if (1 == arguments.length) return this.index.lookup(method, path);
    var args = [method].concat(toArray(arguments));
    if (!this.__usedRouter) this.use(this.router);
    return this.index._route.apply(this.index, args);
  }
});
